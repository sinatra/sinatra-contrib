...
describe Sinatra::Cookies do
  ...
  describe :key? do
    it 'checks request cookies' do
      cookies('foo=bar').key?('foo').should be_truthy
    end

    it 'checks response cookies' do
      jar = cookies
      jar['foo'] = 'bar'
      jar.key?(:foo).should be_truthy
    end

    it 'does not use deleted cookies' do
      jar = cookies('foo=bar')
      jar.delete :foo
      jar.key?('foo').should be_falsey
    end
  end
  ...
  describe :member? do
    it 'checks request cookies' do
      cookies('foo=bar').member?('foo').should be_truthy
    end

    it 'checks response cookies' do
      jar = cookies
      jar['foo'] = 'bar'
      jar.member?(:foo).should be_truthy
    end

    it 'does not use deleted cookies' do
      jar = cookies('foo=bar')
      jar.delete :foo
      jar.member?('foo').should be_falsey
    end
  end
  ...
  describe :value? do
    it 'checks request cookies' do
      cookies('foo=bar').value?('bar').should be_truthy
    end

    it 'checks response cookies' do
      jar = cookies
      jar[:foo] = 'bar'
      jar.value?('bar').should be_truthy
    end

    it 'does not use deleted cookies' do
      jar = cookies('foo=bar')
      jar.delete :foo
      jar.value?('bar').should_not be_truthy
    end
  end
